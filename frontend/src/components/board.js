    ...
    function reportClick(clientX, clientY) {
        ...
        if (selected != null) {
            if (props.inputMode === "move_train") {
                fetch(`/game/${gameId}/actions/move-train/${x}/${y}/`, {method: "POST"}).then((r) => {
                    setNeedToFetch(true)
                    if (r.status !== 200) {
                        r.text().then((errorMsg) => {
                            alert(errorMsg)
                        })
                    }
                })
                ...
                if (areAdjacent(selected, [x, y])) {
                    fetch(`/game/${gameId}/actions/add/track/${x}/${y}/to/${selected[0]}/${selected[1]}/`, {method: "POST"}).then((r) => {
                        setNeedToFetch(true)
                        if (r.status !== 200) {
                            r.text().then((errorMsg) => {
                                alert(errorMsg)
    ...
    return (
        <div id="ui-container" tabIndex={1}
            ...
            <svg id="playboard"
                ...
                <HighlightsLayer actions={actions} gameId={gameId} highlightCity={highlightCity}
                                 highlightGood={highlightGood} highlightMovement={inputMode === "move_train"}/>
            </svg>
        </div>
    )
}
