...
def action_good_pickup(request, game_id, good_id):
    ...
    current_location = get_current_train_location(game_id, slot.id)
    ...
    game_action = actiontypes.good_pickup(game_id, next_sequence_number, slot.id, good_id)
    game_action.save()
...
def action_good_dump(request, game_id, good_id):
    ...
    actiontypes.good_deliver(game_id, next_sequence_number, slot.id, available_to_dump[0], good_id).save()
...
def action_good_deliver(request, game_id, good_id, card_id):
    ...
    pickup_ids_available_to_deliver = get_current_goods_carried(game_id, slot.id)[good_id]
    ...
    actiontypes.money_adjust(game_id, next_sequence_number, slot.id, price).save()

    next_sequence_number += 1

    actiontypes.demand_discarded(game_id, next_sequence_number, slot.id, card_action.sequence_number).save()

    next_sequence_number += 1

    actiontypes.good_deliver(game_id, next_sequence_number, slot.id, pickup_id, good_id).save()
