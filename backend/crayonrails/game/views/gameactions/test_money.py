...
class AdjustMoney(TestCase):
    ...
    def test_anonymous(self):
        ...
        response = action_adjust_money(request, self.game.id, self.slot.id, "plus", 5)
    ...
    def test_wrongplayer(self):
        ...
        response = action_adjust_money(request, self.game.id, self.other_slot.id, "plus", 5)
        self.assertEqual(response.status_code, 403)
    ...
    def test_creator(self):
        ...
        response = action_adjust_money(request, game_id=self.game.id, player_id=self.other_slot.id, sign="plus", amount=5)
        self.assertEqual(response.status_code, 200)

        self.assertEqual(get_money_for_player(game_id=self.game.id, player_id=self.other_slot.id), 5)
    ...
    #     self.assertEqual(response.status_code, 200)
    #
    #     self.assertEqual(get_money_for_player(game_id=self.game.id, player_id=self.slot.index), 5)
    #
