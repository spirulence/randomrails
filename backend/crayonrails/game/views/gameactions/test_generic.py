...
from .generic import actions, action_last
...
class ActionsLast(TestCase):
    ...
    def test_anonymous(self):
        ...
        response = action_last(request, self.game.id)
        self.assertEqual(response.status_code, 403)
    ...
    def test_player(self):
        ...
        response = action_last(request, game_id=self.game.id)
        self.assertEqual(response.status_code, 200)

        response_data = json.loads(response.content)["result"]
        self.assertEqual(response_data["sequenceNumber"], 1)